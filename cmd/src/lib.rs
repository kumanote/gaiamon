...
impl CheckManager {
    pub fn gaiad_endpoint(&mut self, gaiad_endpoint: &str) -> &mut Self {
        self.gaiad_endpoint = gaiad_endpoint.to_owned();
        self
    }
    ...
    fn setup(&mut self) -> tokio::runtime::Runtime {
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .thread_name(format!("{}", self.gaiad_endpoint.as_str()))
            .enable_all()
            .build()
            .expect("Failed to create tokio runtime!");
        if self.check_if_syncing {
            ...
            let checker =
                checker::is_syncing::IsSyncingChecker::new(self.gaiad_endpoint.clone(), receiver);
    ...
    async fn next(&mut self) -> Result<&mut Self> {
        ...
        let latest_block_response = gaiacli::get_client(self.gaiad_endpoint.as_str())
            .lock()
            .await
            .fetch_latest_block()
            .await?;
        ...
        if from_height <= latest_height {
            for height in from_height..=latest_height {
                let block_message: BlockMessage = if height == latest_height {
                    ...
                    let block_response = gaiacli::get_client(self.gaiad_endpoint.as_str())
                        .lock()
                        .await
