...
#[structopt(about = "gaiamon running options")]
struct Opts {
...
fn main() {
    ...
    debug!(
        "Loaded gaia daemon monitoring tool config, config: {:?}",
        config
    );

    // set up gaia client pool
    let mut client_pool = HashMap::new();
    for checker in &config.checkers {
        let endpoint = checker.gaia_grpc_endpoint();
        info!("this tool will connect to {}", endpoint.as_str());
        let client = GaiadClient::new(endpoint.clone());
        client_pool.insert(endpoint, Arc::new(Mutex::new(client)));
    }
    gaiacli::set_client_pool(client_pool);

    if let Err(err) = gaiamon::start() {
        error!("{}", err);
    }
