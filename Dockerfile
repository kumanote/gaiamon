...
FROM alpine:3.15
COPY --from=builder /gaiamon/target/release/gaiamon /usr/local/bin/gaiamon
RUN chmod +x /usr/local/bin/gaiamon
...
RUN addgroup -g ${GROUP_ID} gaiamon; \
    adduser -D -u ${USER_ID} -G gaiamon -h /gaiamon -s "/bin/bash" gaiamon;

# install su-exec
RUN apk add --no-cache su-exec; \
    su-exec gaiamon true;
