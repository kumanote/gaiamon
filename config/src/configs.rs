...
pub struct CheckerConfig {
    pub gaia_grpc_scheme: String,
    pub gaia_grpc_host: String,
...
impl CheckerConfig {
    pub fn gaia_grpc_endpoint(&self) -> String {
        format!(
            "{}://{}:{}",
            self.gaia_grpc_scheme, self.gaia_grpc_host, self.gaia_grpc_port
        )
    }
}
...
impl SelfValidation for CheckerConfig {
    fn validate(&self) -> Result<()> {
        if self.gaia_grpc_scheme != "http" && self.gaia_grpc_scheme != "https" {
            return Err(anyhow!(
                "gaia_grpc_scheme must be either 'http' or 'https'..."
            ));
        }
...
impl FromEnv for CheckerConfig {
    fn from_env() -> Result<Self> {
        Ok(Self {
            gaia_grpc_scheme: "http".to_owned(),
            gaia_grpc_host: "127.0.0.1".to_owned(),
            gaia_grpc_port: 9090,
...
impl TryFrom<CheckerToml> for CheckerConfig {
    ...
    fn try_from(toml: CheckerToml) -> Result<Self> {
        ...
        if let Some(gaia_grpc_scheme) = toml.gaia_grpc_scheme {
            result.gaia_grpc_scheme = gaia_grpc_scheme;
        }
        if let Some(gaia_grpc_host) = toml.gaia_grpc_host {
            result.gaia_grpc_host = gaia_grpc_host;
        }
        if let Some(gaia_grpc_port) = toml.gaia_grpc_port {
            result.gaia_grpc_port = gaia_grpc_port;
        }
