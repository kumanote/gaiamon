...
use anyhow::anyhow;
use anyhow::Context;
use gaia_grpc_client as proto;
use gaia_grpc_client::tonic::{self, transport::Channel, Code};
use logger::prelude::*;

#[derive(Debug, Clone)]
pub struct GaiadClient {
    endpoint: String,
...
impl GaiadClient {
    pub async fn establish_connection(endpoint: &str) -> Result<Channel> {
        tonic::transport::Endpoint::new(endpoint.to_owned())?
            .connect()
            .await
            .with_context(|| format!("failed to connect gaiad grpc endpoint: {}", endpoint))
    }
